...
import type { Profile, Contact, PipelineStage } from "@/types";
...
export default function NewDealPage() {
  ...
  useEffect(() => {
    async function loadData() {
      const [{ data: profilesData }, { data: contactsData }, { data: stagesData }] = await Promise.all([
      ...
      if (stagesData && stagesData.length > 0) {
        ...
        setForm(prev => ({ ...prev, stage_id: stagesData[0].id }));
      }
    }
    loadData();
  }, [supabase]);
  ...
  return (
    <div className="p-8 w-full max-w-none space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Create a new sales opportunity and track its stage in your pipeline.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleCreateDeal} className="space-y-8">
        ...
        <Card className="p-6 border-border shadow-sm space-y-6">
          <h2 className="text-lg font-semibold text-foreground border-b border-border pb-3">Pipeline Stage & Assignment</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
