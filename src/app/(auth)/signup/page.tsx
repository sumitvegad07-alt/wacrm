...
function SignupPageInner() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (data.session) {
      ...
      // plan-aware dashboard, NOT "/" — the root route is the public marketing
      // landing page. /dashboard is inside the dashboard shell, which fires
      // first-load provisioning (territory seed, default roles) on any page.
      ...
      <Card className="w-full max-w-md border-border bg-card">
        ...
        <CardContent>
          <form onSubmit={handleSignup} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="salesUsers" className="text-muted-foreground">
                No. of sales user
              </Label>
              <Input
                id="salesUsers"
                ...
                placeholder="e.g. 5"
                value={salesUsers}
                onChange={(e) => setSalesUsers(e.target.value)}
                required
                className="border-border bg-muted text-foreground placeholder:text-muted-foreground focus-visible:border-primary focus-visible:ring-primary/20"
              />
            </div>
