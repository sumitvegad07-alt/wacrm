...
export function ImportTasksModal({ open, onOpenChange, onImported }: ImportTasksModalProps) {
  ...
  function handleDownloadDemo() {
    const headers = ['title', 'description', 'status', 'priority', ...customFields.map((f) => f.field_name.toLowerCase())];
    const row = ['Follow up call', 'Call the new lead to discuss requirements', 'Pending', 'High', ...customFields.map(() => '')];
    ...
    link.href = url;
    link.setAttribute('download', 'tasks_demo.csv');
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (statusIdx >= 0 && values[statusIdx]) {
        const s = values[statusIdx].trim();
        if (['Pending', 'In Progress', 'Waiting', 'Completed', 'Cancelled'].includes(s)) {
          status = s;
        }
      }
      ...
      if (priorityIdx >= 0 && values[priorityIdx]) {
        const p = values[priorityIdx].trim();
  ...
  async function handleImport() {
    ...
    for (const row of parsedRows) {
      ...
      if (error) {
        ...
        if (row.custom.length > 0) {
          ...
          await supabase.from('task_custom_values').insert(cfInserts);
        }
      }
    }
    ...
    if (imported > 0) {
      toast.success(`${imported} task(s) imported successfully`);
      onImported();
    }
    if (failed > 0) toast.error(`${failed} task(s) failed to import`);
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription>
              ...
              Optional: <code className="rounded bg-muted px-1 text-[11px]">description</code>, <code className="rounded bg-muted px-1 text-[11px]">status</code>, <code className="rounded bg-muted px-1 text-[11px]">priority</code>, and any custom fields.
            </DialogDescription>
            <div className="pt-2">
