...
export function ImportDealsModal({ open, onOpenChange, pipelineId, onImported }: ImportDealsModalProps) {
  ...
  function handleDownloadDemo() {
    const headers = ['title', 'value', 'description', ...customFields.map((f) => f.field_name.toLowerCase())];
    const row = ['New Enterprise Deal', '50000', 'Potential for Q4 closing', ...customFields.map(() => '')];
    ...
    link.href = url;
    link.setAttribute('download', 'deals_demo.csv');
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      rows.push({
        base: {
          title,
          value: valIdx >= 0 && values[valIdx] ? parseFloat(values[valIdx]) : 0,
  ...
  async function handleImport() {
    ...
    // Get the first stage of the pipeline to assign deals to
    ...
    if (!firstStageId) {
      toast.error('Pipeline has no stages. Cannot import.');
      setImporting(false);
      return;
    }
    ...
    for (const row of parsedRows) {
      ...
      if (error) {
        ...
        if (row.custom.length > 0) {
          ...
          await supabase.from('deal_custom_values').insert(cfInserts);
        }
      }
    }
    ...
    if (imported > 0) {
      toast.success(`${imported} deal(s) imported successfully`);
      onImported();
    }
    if (failed > 0) toast.error(`${failed} deal(s) failed to import`);
      ...
      <DialogContent className="flex flex-col gap-0 overflow-hidden border-border/80 bg-popover p-0 text-popover-foreground sm:max-w-xl">
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            <DialogTitle className="text-lg">Import Deals</DialogTitle>
            <DialogDescription>
              Upload a CSV with a required <code className="rounded bg-muted px-1 text-[11px]">title</code> column. 
              Optional: <code className="rounded bg-muted px-1 text-[11px]">value</code>, <code className="rounded bg-muted px-1 text-[11px]">description</code>, and any custom fields. Deals will be placed in the first stage of the pipeline.
            </DialogDescription>
            <div className="pt-2">
        ...
        <DialogFooter className="border-t border-border/80 bg-background/50 px-6 py-4">
          ...
          {!result && (
            <Button disabled={parsedRows.length === 0 || importing} onClick={handleImport}>
              ...
              Import {parsedRows.length} deals
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
