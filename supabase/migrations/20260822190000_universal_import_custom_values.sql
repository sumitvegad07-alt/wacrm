-- ============================================================================
-- Universal Import Framework — custom-field (EAV) values for form-backed modules
--   The importer now generates its field set from the same `custom_fields`
--   config the manual form uses (build-descriptor.ts), so a row can carry a
--   `__custom` object { custom_field_id: value }. This adds a helper that writes
--   those to the module's EAV table, and wires it into the contacts / products /
--   leads branches of import_commit (insert + update).
--   Undo is unaffected: *_custom_values cascade on parent delete (confdeltype=c).
-- ============================================================================

CREATE OR REPLACE FUNCTION public.import_write_custom(
  p_table text, p_fk text, p_record uuid, p_custom jsonb, p_replace boolean
)
RETURNS void LANGUAGE plpgsql SECURITY INVOKER SET search_path = public AS $$
BEGIN
  IF p_custom IS NULL OR jsonb_typeof(p_custom) <> 'object' THEN RETURN; END IF;
  -- p_table / p_fk come only from a registered descriptor (whitelisted); %I quotes them.
  IF p_replace THEN
    EXECUTE format(
      'DELETE FROM public.%I WHERE %I = $1 AND custom_field_id IN (SELECT k::uuid FROM jsonb_object_keys($2) AS k)',
      p_table, p_fk) USING p_record, p_custom;
  END IF;
  EXECUTE format(
    'INSERT INTO public.%I (%I, custom_field_id, value) SELECT $1, (kv.key)::uuid, kv.value '
    || 'FROM jsonb_each_text($2) kv WHERE NULLIF(btrim(kv.value), '''') IS NOT NULL',
    p_table, p_fk) USING p_record, p_custom;
END; $$;
GRANT EXECUTE ON FUNCTION public.import_write_custom(text, text, uuid, jsonb, boolean) TO authenticated;

-- Re-deploy import_commit with __custom (EAV) writes wired into the form-backed
-- branches (contacts / products / leads), on both insert and update.
