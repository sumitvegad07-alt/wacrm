-- Fix two actor bugs in the live pg_cron notification generator
-- (process_notification_outbox, added in 20260918160000). Mirrors the TypeScript
-- catalog fix in src/lib/notifications/catalog.ts.
--
--  1. order_created named "A team member" instead of the rep, because the actor
--     was hard-coded NULL. orders.user_id is the creator (populated on every
--     row) — use it.
--  2. task_completed used the task's user_id (the CREATOR) as the actor. When an
--     admin created the task and a rep completed it, the admin was self-skipped
--     out of the "Task completed" alert (and it would have named the wrong
--     person). The completer is assigned_user_id — use that, falling back to the
--     creator.
--
-- Only the two actor CASE branches change; everything else is re-stated verbatim
-- so this file is the current definition of record.
      ...
      IF ev.event_type IN ('order_created','customer_created','lead_created',
        ...
        v_actor := notif_resolve_profile(ev.account_id,
          CASE ev.event_type WHEN 'expense_created' THEN s->>'employee_id'
                             -- orders.user_id is the creator (was hard-coded NULL).
                             WHEN 'order_created' THEN s->>'user_id'
                             -- task_completed: the actor is the COMPLETER, not the creator.
                             WHEN 'task_completed' THEN COALESCE(s->>'assigned_user_id', s->>'user_id')
                             ELSE s->>'user_id' END);
